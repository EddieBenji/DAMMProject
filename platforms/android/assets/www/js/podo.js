...
function Kalman() {
    ...
    this.onFilteringKalman = function (ech)//signal: signal measured
    {
        ...
        if (isNaN(this.x)) {
            ...
            this.P = 1 / this.C * this.Rv / this.C;
        }
        else {
...
function Pedometer() {
    this.acc_norm = []; // amplitude of the acceleration
    ...
    this.setDistanceGPS = function () {
        ...
        if (isNaN(this.distance) && isNaN(this.distGPS)) {
    ...
    this.onSpeed = function () {
        ...
        var speedGPS = 0;
        if (Boolean(this.isGPSEnabled) && Boolean(this.isGPSReceived)) {
            if (this.timeGPS < 6000 && isNaN(this.distGPS) && isNaN(speedAcc) && isNaN(this.timeGPS)) {
                if (this.timeGPS > 0) {
        ...
        // Mean Speed
        if (this.stepArr[this.stepArr.length - 1] !== 0 && (this.speed !== 0 && isNaN(this.speed))) {
            if (isNaN(this.meanSpeed)) {
    ...
    this.onCalory = function () {
        if (isNaN(this.speed)  && isNaN(this.calory)) {
    ...
    this.onDraw = function (context, widthCanvas, heightCanvas) {
        ...
        if (isNaN(this.speed) == 1) {
            angleCanvas = (Math.PI / 180) * (150 + 0 / vitesseLimite);
